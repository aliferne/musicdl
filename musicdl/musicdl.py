...
class MusicClient:
    ...
    def __init__(
        ...
        # prepare and assert
        music_sources = (
            [music_sources] if isinstance(music_sources, str) else music_sources
        )
        assert (
            isinstance(music_sources, Iterable)
        ...
        self.music_sources = list(
            set(music_sources if music_sources else DEFAULT_MUSIC_SOURCES)
        )
        # init
        ...
        assert self.music_clients, (
            f'Invalid "music_sources", elements in "music_sources" should be in ({", ".join(MusicClientBuilder.REGISTERED_MODULES.keys())})'
        )
    ...
    def printandselectsearchresults(
            ...
            for search_result in per_search_results:
                ...
                print_items.append(
                    [
                        ...
                        colorize(
                            str(search_result.singers)[:12] + "..."
                            if len(str(search_result.singers)) > 15
                            else str(search_result.singers),
                            "singer",
                        ),
                        str(search_result.song_name),
                        str(search_result.file_size)
                        if search_result.ext
                        not in MusicClient.LOSSLESS_QUALITY_DEFINITIONS
                        else colorize(str(search_result.file_size), "flac"),
                        ...
                        colorize(
                            "|".join(
                                [
                                    str(s).upper()
                                    for s in [
                                        str(search_result.source).removesuffix(
                                            "MusicClient"
                                        ),
        ...
        if not print_items:
            self.logger_handle.warning(
                "No songs found from %s" % ", ".join(self.music_sources)
            )
            return []
        print(
            smarttrunctable(
        ...
        selected_rows = [
            id2row[i]
            for i in picked_ids
            if i in (id2row := dict(zip(row_ids, print_items)))
        ]
        print("\nNo songs selected.\n") if not selected_rows else (
            print("\nSelected songs:\n"),
            print(
                smarttrunctable(
    ...
    def startcmdui(self):
        while True:
            ...
            for song_info in selected_song_infos:
                ...
                ) if song_info.episodes else final_selected_song_infos.append(song_info)
    ...
    def search(self, keyword) -> dict[str, list[SongInfo]]:
        ...
        with Progress(
            ...
            main_progress_id = main_process_context.add_task(
                f"Search From Sources >>> Completed (0/0) Search URLs", total=0
            )
    ...
    def download(
        self, song_infos: list[dict[SongInfo]] | dict[str, list[SongInfo]]
    ) -> list[SongInfo]:
        classified_song_infos: dict[str, list[SongInfo]] = {}
        downloaded_song_infos: list[SongInfo] = []
        song_infos: list[SongInfo] = (
            song_infos
            if isinstance(song_infos, list)
            else list(chain.from_iterable(song_infos.values()))
        )
        for song_info in song_infos:
            classified_song_infos.setdefault(song_info.source, []).append(song_info)
        for source, source_song_infos in classified_song_infos.items():
            downloaded_song_infos.extend(
    ...
    def parseplaylist(self, playlist_url: str) -> list[SongInfo]:
        ...
        for source in list(self.music_clients.keys()):
            with suppress(Exception):
                song_infos = self.music_clients[source].parseplaylist(
                    playlist_url, request_overrides=self.requests_overrides[source]
                )
...
@click.option(
    ...
    "--playlist_url",
    default=None,
    help='Given a playlist URL, e.g., "https://music.163.com/#/playlist?id=7583298906", musicdl automatically parses the playlist and downloads all tracks in it.',
    type=str,
    show_default=True,
)
@click.option( 
...
def MusicClientCMD(
    ...
    # parse playlist url
    assert keyword is None or playlist_url is None, (
        '"playlist_url" and "keyword" could not be set simultaneously'
    )
    # load json string
    ...
    music_sources = [
        ms.strip() for ms in music_sources.replace(" ", "").split(",") if ms.strip()
    ]
