...
SIGN_KEY_SECRET = (
    ...
    if IS_LITE
    else "57ae12eb6890223e355ccfcb74edf70d"
)
SIGNATURE_ANDROID_SECRET = (
    "LnT6xpN3khm36zse0QzvmgTZ3waWdRSA" if IS_LITE else "OIlwieks28dk2k092lksi2UIkp"
)
PUBLIC_RSA_KEY = (
    """-----BEGIN PUBLIC KEY-----
...
class KugouMusicClientUtils:
    ...
    def signatureandroid(params: Dict[str, Any], data: str = "") -> str:
        params_string = "".join(
            f"{k}={json.dumps(params[k], separators=(',', ':'), ensure_ascii=False) if isinstance(params[k], (dict, list)) else params[k]}"
            for k in sorted(params.keys())
        )
        return KugouMusicClientUtils.md5hex(
            f"{SIGNATURE_ANDROID_SECRET}{params_string}{data}{SIGNATURE_ANDROID_SECRET}"
        )

    """signatureandroidwithsecret"""

    @staticmethod
    def signatureandroidwithsecret(
        params: Dict[str, Any], data: str, secret: str = "OIlwieks28dk2k092lksi2UIkp"
    ) -> str:
        params_string = "".join(
            f"{k}={json.dumps(params[k], separators=(',', ':'), ensure_ascii=False) if isinstance(params[k], (dict, list)) else params[k]}"
            for k in sorted(params.keys())
    ...
    def signatureweb(params: Dict[str, Any]) -> str:
        ...
        return KugouMusicClientUtils.md5hex(
            f"{SIGNATURE_WEB_SECRET}{params_string}{SIGNATURE_WEB_SECRET}"
        )
    ...
    def signkey(hash_value: str, mid: str, userid: str, appid: str) -> str:
        return KugouMusicClientUtils.md5hex(
            f"{hash_value}{SIGN_KEY_SECRET}{appid}{mid}{userid or 0}"
        )
    ...
    def initdevice(cookies: dict = None):
        cookies, mid = (
            cookies or {},
            KugouMusicClientUtils.calculatemid((guid := str(uuid.uuid4()))),
        )
        (
            cookies["KUGOU_API_GUID"],
            cookies["KUGOU_API_MID"],
            cookies["KUGOU_API_MAC"],
            cookies["KUGOU_API_DEV"],
        ) = (
            guid,
            mid,
            KugouMusicClientUtils.randomstring(12),
            KugouMusicClientUtils.randomstring(16),
        )
        return cookies
    ...
    def sendrequest(
        ...
        # init
        clienttime, params, headers, request_overrides = (
            int(time.time()),
            params or {},
            headers or {},
        ...
        token, dfid, userid, mid = (
            used_cookies.get("token", ""),
            used_cookies.get("dfid", "-"),
            used_cookies.get("userid", 0),
            used_cookies.get("KUGOU_API_MID", "-"),
        )
        # construct params
        default_params = {
            "dfid": dfid,
        ...
        # encrypt key
        if encrypt_key:
            params["key"] = KugouMusicClientUtils.signkey(
                params["hash"], params["mid"], params.get("userid"), params["appid"]
            )
        # signature
        data_str = (
            json.dumps(data, separators=(",", ":"), ensure_ascii=False)
        ...
        params.pop("signature", None) if not_sign and "signature" in params else (
            params.__setitem__(
                "signature",
                KugouMusicClientUtils.signatureweb(params)
                if encrypt_type == "web"
                else KugouMusicClientUtils.signatureandroid(params, data_str),
            )
            if (not not_sign and "signature" not in params)
            else None
        )
        # construct headers
        final_headers = {
    ...
    @staticmethod
    def registerdevice(
        session: requests.Session, cookies: dict, request_overrides: dict = None
    ):
        # construct
        data_map = {
            ...
            "buildSerial": "unknown",
            "device": "marble",
            "imei": cookies.get("KUGOU_API_GUID"),
            "imsi": "",
            "manufacturer": "Xiaomi",
            "uuid": cookies.get("KUGOU_API_GUID"),
            "accelerometerValue": "",
            "gravity": False,
            "gravityValue": "",
            "gyroscope": False,
        ...
        # aes
        aes_key = KugouMusicClientUtils.randomstring(6).lower()
        encrypt_key = KugouMusicClientUtils.md5hex(aes_key)[:16]
        encrypt_iv = KugouMusicClientUtils.md5hex(aes_key)[16:32]
        raw = json.dumps(data_map, separators=(",", ":"), ensure_ascii=False).encode(
            "utf-8"
        )
        enc = AES.new(
            encrypt_key.encode("utf-8"), AES.MODE_CBC, encrypt_iv.encode("utf-8")
        ).encrypt(KugouMusicClientUtils.pad(raw))
        p = KugouMusicClientUtils.rsaencryptpkcs1(
            {
                "aes": aes_key,
                "uid": cookies.get("userid", 0),
                "token": cookies.get("token", ""),
            }
        )
        # send request and return result
        resp_raw: bytes = KugouMusicClientUtils.sendrequest(
            session,
            "POST",
        ...
        try:
            result = (
                json.loads(text)
                if (text := resp_raw.decode("utf-8")).startswith("{")
                else None
            )
        except Exception:
            result = None
        ...
        result: dict = json.loads(
            KugouMusicClientUtils.unpad(dec_cipher.decrypt(resp_raw)).decode("utf-8")
        )
        if result.get("status") == 1 and safeextractfromdict(
            result, ["data", "dfid"], None
        ):
            cookies["dfid"] = result["data"]["dfid"]
        return result
    ...
    def getsongurl(
        ...
        params = {
            ...
            "quality": quality,
            "album_audio_id": int(album_audio_id),
            "behavior": "play",
            "pid": 2 if not IS_LITE else 411,
            "cmd": 26,
            "pidversion": 3001,
            "IsFreePart": 1 if free_part else 0,
            "ppage_id": "463467626,350369493,788954147"
            if not IS_LITE
            else "356753938,823673182,967485191",
            "cdnBackup": 1,
            "kcard": 0,
            "module": "",
        }
        return KugouMusicClientUtils.sendrequest(
            session,
            "GET",
            "/v5/url",
            params=params,
            ...
            encrypt_key=True,
            cookies=cookies,
            cookies_override={"dfid": KugouMusicClientUtils.randomstring(24)},
            request_overrides=request_overrides,
        )
