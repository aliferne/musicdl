...
def legalizestring(
    ...
    max_len: int = 255,
    fit_utf8: bool = True,
    replace_null_string: str = "NULL",
):
    # naive process
    if not string:
        return replace_null_string
    string = re.sub(
        r"\\/>", "/>", re.sub(r"<\\/", "</", str(string).replace(r"\"", '"'))
    )
    string = re.sub(r"\\u([0-9a-fA-F]{4})", lambda m: chr(int(m.group(1), 16)), string)
    # html.unescape
    string = (
        n1
        if (n1 := html.unescape(string)) == string
        else (n2 if (n2 := html.unescape(n1)) != n1 else n1)
    )
    # bleach.clean
    try:
    ...
    # fix encoding
    if fit_gbk:
        string = string.encode("gb18030", errors="ignore").decode(
    ...
    return (
        replace_null_string
        if not (string := re.sub(r"\s+", " ", string).strip())
        else string
    )
...
def resp2json(resp: requests.Response) -> dict:
    if not isinstance(
        resp,
        (
            (requests.Response, curl_cffi.requests.Response)
            if curl_cffi
    ...
    try:
        ...
        return json_repair.loads(resp.text) or {}
...
def isvalidresp(
    resp: requests.Response, valid_status_codes: list | tuple | set = {200, 206}
):
    if not isinstance(
        resp,
        (
...
def safeextractfromdict(data, progressive_keys, default_value=None):
    try:
        result = functools.reduce(lambda x, k: x[k], progressive_keys, data)
    except Exception:
...
def usedownloadheaderscookies(func):
    ...
    def wrapper(self: ClientProtocolObj, *args, **kwargs):
        ...
        if hasattr(self, "enable_download_curl_cffi"):
            self.enable_curl_cffi = self.enable_download_curl_cffi
        if hasattr(self, "_initsession"):
            self._initsession()
...
def useparseheaderscookies(func):
    ...
    def wrapper(self: ClientProtocolObj, *args, **kwargs):
        ...
        if hasattr(self, "enable_parse_curl_cffi"):
...
def usesearchheaderscookies(func):
    ...
    def wrapper(self: ClientProtocolObj, *args, **kwargs):
        ...
        if hasattr(self, "enable_search_curl_cffi"):
...
def hashablesth(obj):
    return (
        tuple((k, hashablesth(v)) for k, v in sorted(obj.items()))
        if isinstance(obj, dict)
        else tuple(hashablesth(x) for x in obj)
        if isinstance(obj, list)
        else tuple(sorted(hashablesth(x) for x in obj))
...
def dedupkeeporder(seq):
    seen, out = set(), []
    for item in seq:
        (seen.add(key), out.append(item)) if (
            key := hashablesth(item)
        ) not in seen else None
    return out
...
def searchdictbykey(obj, target_key: str | list | tuple | set) -> list:
    results, target_key = (
        [],
        [target_key] if isinstance(target_key, str) else target_key,
    )
    if isinstance(obj, dict):
        for k, v in obj.items():
            results += ([v] if k in target_key else []) + searchdictbykey(v, target_key)
    elif isinstance(obj, list):
        for item in obj:
            results.extend(searchdictbykey(item, target_key))
...
class IOUtils:
    ...
    def touchdir(
        ...
        if auto_sanitize:
            directory = sanitize_filepath(directory)
    ...
    def replacefile(src: str, dest: str):
        try:
            ...
            (exc.errno == errno.EXDEV or (_ for _ in ()).throw(Exception))
            (
                not os.path.exists(dest)
                or (
                    (not os.path.isdir(dest) and (os.remove(dest) or True))
...
class AudioLinkTester:
    ...
    def chooseaudioextfromffprobeoutput(codec_name: str | None) -> str:
        ...
        return (
            mapping[codec_name]
            if (codec_name := codec_name.lower()) in mapping
            else ".wav"
    ...
    def extractaudiofromvideolossless(video_path, audio_path: str | None = None) -> str:
        ...
        ext = AudioLinkTester.chooseaudioextfromffprobeoutput(
            AudioLinkTester.ffprobeaudiocodec(str(video_path))
        )
        audio_path = str(
            (Path(audio_path) if audio_path is not None else video_path).with_suffix(
                ext
            )
        )
        cmd = ExtractAudioFromVideoFFmpegCommand().build(
            video_path=str(video_path), audio_path=audio_path
        )
        try:
            subprocess.run(cmd, capture_output=True, text=True, check=True)
            return audio_path, ext.removeprefix(".")
        except subprocess.CalledProcessError:
            subprocess.run(
                ExtractAudioFromVideoFFmpegCommand().build(
                    str(video_path),
                    (fallback_path := str(video_path.with_suffix(".mka"))),
                ),
                capture_output=True,
                text=True,
                check=True,
            )
            return fallback_path, "mka"
    ...
    def byte2mb(size: int):
        try:
            size = (
                "NULL"
                if not (mb := round(int(float(size)) / 1024 / 1024, 2))
                else f"{mb} MB"
            )
        except Exception:
            size = "NULL"
    ...
    def isaudiomime(cls, content_type: Optional[str]) -> bool:
        if not (ctype := cls.normalizectype(content_type)):
            return False
    ...
    def normalizeext(cls, ext: Optional[str]) -> Optional[str]:
        if not ext:
            return None
        if not (ext := ext.strip().lower().lstrip(".")):
            return None
    ...
    def parsesizefromheaders(headers: CaseInsensitiveDict) -> Optional[int]:
        if (content_length := headers.get("Content-Length")) and str(
            content_length
        ).isdigit():
            return int(content_length)
        if "/" in (content_range := headers.get("Content-Range", "") or ""):
            return (
                int(total)
                if (total := content_range.rsplit("/", 1)[-1].strip()).isdigit()
                else None
            )
        return None
    ...
    def extractfilenamefromcd(content_disposition: Optional[str]) -> Optional[str]:
        ...
        for part in [p.strip() for p in content_disposition.split(";")]:
            if (key := part.lower()).startswith("filename="):
                return part.split("=", 1)[1].strip('"')
            if key.startswith("filename*="):
                return unquote(
                    (
                        v.split("''", 1)[1]
                        if "''" in (v := part.split("=", 1)[1])
                        else v
                    ).strip('"')
                )
        return None
    ...
    def sampleresponsebytes(resp: requests.Response, max_bytes: int = 8192) -> bytes:
        ...
        for chunk in resp.iter_content(chunk_size=4096):
            if not chunk:
                continue
            if (remain := max_bytes - total) <= 0:
                break
            chunks.append((piece := chunk[:remain]))
            if (total := total + len(piece)) >= max_bytes:
                break
    ...
    def request(
        ...
        headers = dict(
            (kwargs := self.buildrequestkwargs(request_overrides)).get("headers") or {}
        )
        if method.upper() == "GET" and range_bytes:
        ...
        return {
            ...
            "file_size_bytes": self.parsesizefromheaders(resp.headers),
        }
    ...
    def inferext(
        ...
        # 1) URL suffix first
        candidates.append(
            ("urllib(original_url_suffix)", self.extractsuffixfromurl(original_url))
        )
        candidates.append(
            ("urllib(final_url_suffix)", self.extractsuffixfromurl(final_url))
        )
        if cd_name := self.extractfilenamefromcd(content_disposition):
            candidates.append(
                (
                    "urllib(content_disposition_filename)",
                    os.path.splitext(cd_name)[1].lstrip("."),
                )
            )
        # 2) mimetypes
        candidates.append(
            (
                "mimetypes.guess_type(original_url)",
                self.MIME_TO_PREFERRED_EXT.get(
                    (mimetypes.guess_type(original_url)[0] or "").lower()
                ),
            )
        )
        candidates.append(
            (
                "mimetypes.guess_type(final_url)",
                self.MIME_TO_PREFERRED_EXT.get(
                    (mimetypes.guess_type(final_url or "")[0] or "").lower()
                ),
            )
        )
        candidates.append(
            (
                "mimetypes.from_content_type",
                self.MIME_TO_PREFERRED_EXT.get(
                    (normalized_ctype := self.normalizectype(content_type))
                ),
            )
        )
        if normalized_ctype:
            ext = (
                guessed.lstrip(".")
                if (guessed := mimetypes.guess_extension(normalized_ctype))
                else None
            )
            candidates.append(
                (
                    ...
                    (
                        "m4a"
                        if ext == "mp4"
                        and normalized_ctype in {"audio/mp4", "video/mp4"}
                        else ext
                    ),
                )
            )
        for strategy, raw_ext in candidates:
            if self.isvalidaudioext(ext := self.normalizeext(raw_ext)):
                return ext, strategy
            reason.append(f"{strategy} -> {raw_ext!r} (invalid or non-audio ext)")
        # 3) byte sniff fallback
        if not sample_bytes:
            reason.append("byte sniff skipped: empty sample bytes")
            return None, "NULL"
        try:
            ext = self.normalizeext(
                getattr(guess, "extension", None)
                if (guess := filetype.guess(sample_bytes))
                else None
            )
            ext = "m4a" if ext == "mp4" and self.isaudiomime(content_type) else ext
            if self.isvalidaudioext(ext):
                return ext, "filetype.guess(bytes)"
            reason.append(
                f"filetype.guess(bytes) -> {ext!r} (invalid or non-audio ext)"
            )
        ...
        try:
            ...
            if matches:
                raw_ext = getattr(matches[0], "extension", None) or getattr(
                    matches[0], "name", None
                )
            ext = (
                "m4a"
                if (ext := self.normalizeext(raw_ext)) == "mp4"
                and self.isaudiomime(content_type)
                else ext
            )
            if self.isvalidaudioext(ext):
                return ext, "puremagic.from_string(bytes)"
            reason.append(
                f"puremagic.from_string(bytes) -> {ext!r} (invalid or non-audio ext)"
            )
        except Exception as exc:
    ...
    def test(
        ...
        try:
            head_resp: requests.Response = (
                head_info := self.request(
                    "HEAD", url, request_overrides=request_overrides
                )
            )["resp"]
            result["status_code"], result["download_url"] = (
                head_info["status_code"],
                head_info["download_url"] or url,
            )
            result["ctype"], result["file_size_bytes"] = (
                head_info["ctype"] or "NULL",
                head_info["file_size_bytes"] or 0,
            )
            result["file_size"] = AudioLinkTester.byte2mb(result["file_size_bytes"])
            if not head_info["ok"]:
                reason.append(
                    f"HEAD returned non-2xx status: {head_info['status_code']}"
                )
                result["method"] = "requests.head"
                head_resp.close()
            else:
                reason.append(
                    f"HEAD success: status={head_info['status_code']}, ctype={head_info['ctype']!r}, final_url={head_info['download_url']!r}"
                )
                ext, ext_method = self.inferext(
                    original_url=url,
                    final_url=head_info["download_url"],
                    content_type=head_info["ctype"],
                ...
                head_resp.close()
                if ext:
                    result["ext"], result["ok"], result["method"], result["reason"] = (
                        ext,
                        True,
                        f"requests.head + {ext_method}",
                        reason,
                    )
                    return result
                reason.append(
                    "HEAD succeeded but could not infer a valid audio ext; fallback to GET."
                )
        except Exception as exc:
        ...
        try:
            get_resp: requests.Response = (
                get_info := self.request(
                    "GET", url, request_overrides=request_overrides
                )
            )["resp"]
            result["status_code"], result["download_url"] = (
                get_info["status_code"],
                get_info["download_url"] or url,
            )
            result["ctype"], result["file_size_bytes"] = (
                get_info["ctype"] or "NULL",
                get_info["file_size_bytes"] or 0,
            )
            result["file_size"] = AudioLinkTester.byte2mb(result["file_size_bytes"])
            if not get_info["ok"]:
                reason.append(f"GET returned non-2xx status: {get_info['status_code']}")
                result["method"] = "requests.get"
                get_resp.close()
            else:
                reason.append(
                    f"GET success: status={get_info['status_code']}, ctype={get_info['ctype']!r}, final_url={get_info['download_url']!r}"
                )
                sample_bytes = self.sampleresponsebytes(get_resp, max_bytes=8192)
                ext, ext_method = self.inferext(
                    original_url=url,
                    final_url=get_info["download_url"],
                    content_type=get_info["ctype"],
                ...
                get_resp.close()
                if ext:
                    result["ext"], result["ok"], result["method"] = (
                        ext,
                        True,
                        f"requests.get(stream) + {ext_method}",
                    )
                else:
                    reason.append(
                        "GET succeeded but still could not infer a valid audio ext."
                    )
                    result["method"] = "requests.get(stream) + ext_inference_failed"
        except Exception as exc:
            reason.append(f"GET error: {exc}")
        # return
        result["reason"] = reason
        if not result["method"]:
            result["method"] = "HEAD -> GET"
        return result
